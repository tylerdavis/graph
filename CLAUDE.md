...
- **Tool catalog namespacing**: `<server>__` (MCP), `user__` (user tools), `builtin__` (bundled packs), `plan__` (plan docs), plus reserved bare names `plan_and_execute`, `exit`, and `decide`. Everything shares one `ToolRegistry` catalog (`CompositeRegistry` merges sources); shape recording wraps the base. Packs (`[tools].packs`, YAML embedded in `graph-core/src/packs/`) reuse the user-tool format and registry with a different prefix; customizing one means copying it into a tools dir as a `user__` tool.
...
- **`exit` gates and `decide` forks** are executor-intercepted steps (never dispatched to a registry), sharing one gate grammar (`pipeline/condition.rs`): `when` (logical value/op/to) or `infer` (yes/no verdict via the `judge` role). Fired exits skip remaining steps *and the solver*; error exits → process exit code 4, `is_error` plan-tool results. `decide` runs `then`/`else` (single call or inline step list): only the condition renders up front and only the chosen branch renders at all; branch-step results stay scoped (never in `state.results` — the step cursor and replan merge key on it) but count in `steps_executed`; no `exit`/`decide` nesting inside branches — branches call `plan__*` for that.
