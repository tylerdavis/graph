...
**graph dogfoods itself on every PR** (`.github/workflows/graph-checks.yaml`): the `docs_drift` plan fails the check (exit 4) on undocumented crate behavior changes, and `graph_review_9000` maintains eight passes' worth of findings over the diff plus full file contents as living review threads (prior findings read back by hidden payload marker, fixed ones resolved with a reply, human-dismissed ones treated as declined, only new ones posted) plus a marker-keyed summary comment — both run from the repo-carried `./.graph/` inside the release image (needs the `ANTHROPIC_API_KEY` repo secret). `graph_review_9000` is a composition: `plan__graph_review_core` is the locally runnable review engine (git-only, over the worktree-capable `user__repo_*` tools in `.graph/tools/`) and `plan__graph_review_thread_audit` settles thread lifecycle, called via a decide gate only when prior findings exist. `.graph/` is partially tracked: config, plans, and tools are committed; everything else under it stays ignored. Expect the drift gate to hold you to the docs-parity invariant below.
