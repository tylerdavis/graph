...
prepare() {
  ...
  # must act) once into docs/snippets/changelog/graph/<version>/ — the
  # files the review below is for; they are curated by hand from then on
  # (a migration prompt that belongs to a bumped file kind moves to that
...
require_prepared() {
  ...
  [ -d "docs/snippets/changelog/graph/v$new" ] || { echo "docs/snippets/changelog/graph/v$new/ is missing — prepare did not finish" >&2; exit 1; }
}
