...
# must act) once and persists it under changelog.d/ — review those files
# before pushing; they are curated by hand from then on — and
# compose_changelog rebuilds docs/changelog.mdx in full from the release
# context plus every committed snippet, so the page can never drift from
