...
- An `infer` gate (on `exit` or `decide`) uses the `judge` model role by default; add `model` (any configured model role) alongside `infer` to pin that one verdict to a cheaper or stronger model.
