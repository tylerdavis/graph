...
pub fn map_tool_def() -> crate::tools::ToolDef {
    crate::tools::ToolDef {
        ...
        description: "Run the same body once per item of a list. `over` must produce an \
                      ...
                      1 only when the per-item calls are independent. The body may contain an \
                      `agent` step; it may not contain `exit`, `decide`, `map`, or \
                      `reduce` — call a plan (plan__*) for nested control flow."
            .to_string(),
        input_schema: json!({
            "type": "object",
            "required": ["over", "do"],
...
pub fn reduce_tool_def() -> crate::tools::ToolDef {
    crate::tools::ToolDef {
        ...
        description: "Fold a list into a single value. `over` must produce an array; the \
                      ...
                      reduce over its results. The body may contain an `agent` step; it \
                      may not contain `exit`, `decide`, \
