...
- Branches may contain `exit` steps — a fired exit ends the WHOLE plan from inside the branch (e.g. then: post a comment and exit success) — and `agent` or `ask` steps, but never `decide`, `map`, or `reduce`; use a plan__* call inside the branch for nested control flow.
...
- Bodies may contain `agent` and `ask` steps, but never `exit`, `decide`, `map`, or `reduce`; use a plan__* call inside the body for nested control flow. An `ask` inside a `map` body asks once per item, in item order even when `concurrency` is above 1 — prefer asking once about the whole list."#;
