...
Two scoping rules come from [branching](/plans/branching): a `decide` step's result is referenced through its own id (`{{E1.result.…}}`, `{{E1.branch}}`) — branch-internal step ids are visible only to later steps _inside the same branch_ — and the non-taken branch's templates are never rendered at all.
