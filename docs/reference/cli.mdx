...
`migrate` rewrites one plan file to the current [file version](/reference/file-versions#plans-and-tools), resolving a path or an identifier the same lenient way `validate` does. A file already at the current version is left untouched.
...
`migrate` rewrites one user tool file to the current [file version](/reference/file-versions#plans-and-tools). It takes a path rather than a catalog name, since a file the catalog refuses is the one most likely to need it, and it never starts the tool catalog (no MCP connections).
...
graph config path     # config file locations, which exist, and their version
graph config check    # each file's version and whether the merged config loads
graph config migrate  # rewrite ./.graph/config.toml to the current version (--global for ~/.config/graph/)
```

`init` refuses to overwrite an existing config; `--force` overwrites it.

`check` is the pre-flight for a CI job or a fresh binary: it reports every config file's [file version](/reference/file-versions#config) — `current`, `migrate`, `too new`, or `too old` — then loads the merged config and exits `1` with the problems if it does not load. `migrate` preserves comments and layout, writing `version` as the first key; a file already at the current version is left untouched.
...
Prints the binary version and the [file version](/reference/file-versions) each kind of file is written at; `--json` adds, under `fileVersions`, the oldest version the binary still reads. `--version` prints the same two lines; `-V` prints the binary version alone.
