...
graph plan migrate <name|path> [--json]
graph plan run <name> [<json>|@file|-] [--input k=v]... [--json]
```

`migrate` rewrites one plan file to the current [plan format](/reference/formats#plans-and-tools), resolving a path or an identifier the same lenient way `validate` does. A file already at the current format is left untouched.
...
`new` scaffolds a plan with no steps. It is intentionally invalid on creation (`plan has no steps`) — an edit is only refused if it makes things _worse_, so a scaffold stays editable and a plan can be built up step by step without ever invoking the planner.
...
Exit codes: `0` applied, `1` rejected. With `--json` the envelope is printed to stdout _before_ the non-zero exit, so a caller always gets the structured problem list. Without it, a one-line result goes to stderr and stdout stays empty — see the [scripting contract](/reference/scripting-contract).

Changing `identifier` writes a _new_ file (`<plans dir>/<new>.yaml`) and leaves the original in place, reporting `renamedFrom`; removing the old file is left to you, so a rename can never silently destroy a plan.
...
graph tools migrate <path> [--json]
```

`migrate` rewrites one user tool file to the current [tool format](/reference/formats#plans-and-tools). It takes a path rather than a catalog name, since a file the catalog refuses is the one most likely to need it, and it never starts the tool catalog (no MCP connections).
...
`test --json` wraps the call: `{tool, isError, result}`. A tool that _reports_ an error still exits `0` — the call completed and its payload is the result worth reading, so `isError` carries the distinction the exit code does not. Without `--json` the tool's own result is the deliverable, unwrapped.
...
graph config path     # config file locations, which exist, and their format
graph config check    # each file's format and whether the merged config loads
graph config migrate  # rewrite ./.graph/config.toml to the current format (--global for ~/.config/graph/)
```

`init` refuses to overwrite an existing config; `--force` overwrites it.

`check` is the pre-flight for a CI job or a fresh binary: it reports every config file's [format version](/reference/formats#config) — `current`, `migrate`, or `too new` — then loads the merged config and exits `1` with the problems if it does not load. `migrate` preserves comments and layout, writing `format_version` as the first key; a file already at the current format is left untouched.
...
Prints the binary version and the [format version](/reference/formats) each kind of file is written at; `--json` adds the oldest format the binary still reads. `--version` prints the same two lines; `-V` prints the binary version alone.
