...
`draft` runs the planner over your tool catalog. A draft that fails validation gets one automatic repair pass; if incremental drafting exhausts its retries, the valid prefix is saved and reported under `salvaged` so you can finish it with `step add` instead of redrafting. `--stdout` prints the YAML and writes nothing.
