...
Schemas are deliberately not included: `hasOutputSchema` tells you whether one exists, and `graph tools show <name>` prints it. For what a tool *really* returns — as opposed to what it declares — read the [shape cache](/tools/shape-cache) with `graph shapes show <name>`.
