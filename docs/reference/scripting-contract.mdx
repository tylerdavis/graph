...
Other keys worth branching on: `availableSteps` (an unknown step id, listing what exists), `renamedFrom` (an `identifier` change wrote a new file and left the original), and `salvaged` / `failedStep` (`draft` saved a valid prefix after incremental drafting ran out of retries).
