...
Each candidate names its own provider **and** model (model names rarely carry across providers); `temperature` optionally overrides the request's temperature — otherwise the primary's effective temperature carries over. Every referenced provider must exist under `[providers]`; startup fails otherwise, so a typo'd fallback surfaces immediately rather than mid-outage.

Failover semantics:

- A call moves to the next candidate only on **outage-shaped errors** — the same transient class the retry layer recognizes (429, 5xx, connection failures, timeouts) — and only after the failing provider's own retries (up to 3 attempts with backoff) are exhausted. Permanent errors (4xx, parse/schema failures) propagate immediately: a bad request would fail everywhere, and a fallback would only mask it.
- Streaming follows the same contract as retries: failover happens only while the stream is being established. Once tokens flow, a mid-stream error surfaces as-is.
- Fallbacks apply wherever the entry resolves — the `chat` agent loop, pipeline roles (planner/solver/repair/judge), structured output, and named-model calls. Each failover is logged as a warning on stderr.
