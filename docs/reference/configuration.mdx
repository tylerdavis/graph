...
| `timeout_secs` | `OTEL_EXPORTER_OTLP_TIMEOUT` (milliseconds) | per-export request timeout, default 30 s; a batch that exceeds it is dropped with a warning on stderr |
...
**What a trace contains.** Each `plan run`, `ask` turn, `chat` turn, MCP `plan_run` call, workbench plan run, workbench chat turn, and `plan draft` is one trace, named after the plan identifier or the command. Under the root: one span per plan step and body call (named `E3 user__git_log`, `E1/do.2/E5 builtin__infer`, keyed by the same bus path the workbench and `GRAPH_EVENTS=jsonl` use; a nested plan's steps sit under the step that called it; the tool call a step makes *is* that span), one span per tool call an `ask`/`chat`/workbench agent makes (a `plan__*` call there holds the plan's steps; a `workbench__draft_plan` call holds one `draft step N` span per drafted step, with each rejected attempt as an event on it), and one `generation` span per billable model call under the step that spent it, named after the model role (`chat`, `planner`, `solver`, a custom role) — the same attribution `by_step` reports. Every generation carries the standard `gen_ai.*` attributes: provider, model, input/output/cache token counts, and, when `[pricing]` prices the model, the cost. Errors mark their span and set its status. The `usage_summary` totals land on the root span, and so does the run's own input and output when `capture_content` is on: the plan input and its answer or output document (or the failure), the `ask` message and the reply.

**Sessions.** Every span carries a `session.id` where one applies, so a backend that groups traces into sessions ([Langfuse does](https://langfuse.com/docs/observability/features/sessions)) shows a conversation or a workbench sitting as one session of many traces: for `ask` and `chat` the session is the thread id and each turn is a trace (a new thread is created before the first turn so its id is on every span, and removed again if that turn fails); for the workbench the session is a fresh id per launch and each plan run and each chat turn is a trace. `plan run` and `plan draft` have no session.

**Backend notes.** Langfuse reads these spans as-is: generations, usage, cost, `session.id`, `user.id` (`[user].name`), and the `plan:<identifier>` / `command:<name>` tags are all on every span, as its filters require. Content is what makes a trace readable, so it is on by default; set `capture_content = false` for a backend that must not see what the tools returned. When on, it is written once per span under the `langfuse.observation.input` / `langfuse.observation.output` keys Langfuse maps, each capped at 64 KiB. A value over the cap stays valid JSON: its long strings are cut with a `…[N bytes truncated]` marker, so a backend still renders the structure. Other backends keep the `langfuse.*` keys as plain metadata. Export runs in the background in batches of 16 spans every 2 s (about 2 MB at most, under Langfuse's request ceiling), and the process drains what is left before it exits (up to two minutes); `graph mcp serve` exports continuously. An export that fails or times out is reported as a warning on stderr and that batch is lost.
