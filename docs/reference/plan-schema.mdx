...
The complete field reference for plan YAML files. Concepts and worked examples live in the [Plans](/plans/overview) section.

## Top level

| Field | Required | Type | Notes |
| --- | --- | --- | --- |
| `format_version` | no | integer | the [plan format](/reference/formats) the file is written in; omitted means `1`. Files graph writes always carry it |
...
| `requires_graph` | no | string | semver requirement on the graph version (`">=0.13.0"`); unmet → plan hidden, loud error by name ([formats](/reference/formats#requires_graph)) |
...
Seven reserved `tool_name`s are executor-intercepted, never dispatched to a registry. Their `input` is the control grammar — snake_case like everything else, and the only part of a step's `input` graph normalizes, since every other tool's arguments belong to that tool:
