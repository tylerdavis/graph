...
Field names here are the canonical spelling: snake_case, and that is what anything writing a plan file emits — `graph plan show`, the [authoring commands](/reference/cli#authoring-plans), and the [workbench's](/workbench/plan-workbench) save. The camelCase spellings the planner uses internally (`toolName`, `queryToAnswer`) are accepted as aliases on the way in, so a hand-written plan and a generated one are the same file format. Optional fields you never set are omitted rather than written as `null`/`[]`.
...
Five reserved `tool_name`s are executor-intercepted, never dispatched to a registry. Their `input` is the control grammar. Field names accept both the camelCase spelling the planner emits and the snake_case one plan files use (as `tool_name` itself does):
