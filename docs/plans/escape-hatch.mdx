...
In the [workbench](/using/plan-workbench), a fired gate is visible at a glance — everything it pre-empted shows `⊘` skipped:

<WorkbenchShot
  shot="exit-gate"
  caption="A fired success exit: the gate held, so the diff step and the solver never ran — and the pane says so."
/>
