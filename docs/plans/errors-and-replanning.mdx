...
1. **Load time** (automatically on every load, everywhere): YAML shape, template syntax, forward references (`E0` referencing `E2`), id format, control-step inputs (a `decide` gate's arity, a `map`'s body, an [`agent`](/plans/agent-step)'s `outputSchema` and budget — inside bodies too), malformed tool names — including `workbench__*`, which is rejected outright: those tools exist only inside the workbench TUI's chat agent, never in the plan runtime, so a step naming one is always a defect. This layer is static and context-free.
