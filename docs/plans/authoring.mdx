...
Steps run **sequentially**; each input may reference plan inputs and any earlier step. Tool names are namespaced catalog names, plus the four bare control steps — [`exit`](/plans/exit-gates), [`decide`](/plans/branching), and [`map`/`reduce`](/plans/iteration) — and [`plan_and_execute`](/plans/the-planner), the reserved planner tool.
...
<Tip>
  ...
  This installs [/graph-plan-authoring](https://github.com/tylerdavis/graph/blob/main/skills/graph-plan-authoring/SKILL.md) (Claude Code, Cursor, Codex, and most other agents), which teaches the whole command surface: probe the tools, scaffold or draft, build the steps up one edit at a time, and read the rejection envelopes as a repair list.
</Tip>
