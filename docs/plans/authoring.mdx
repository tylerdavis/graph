...
Validation catches structural problems at load time: template syntax errors, references to steps that haven't run yet (`E2` referencing `E5`), bad ids, unknown fields. What it *can't* catch — a path that doesn't exist in a tool's real output — surfaces at run time as a typed error with the available keys listed. See [Errors & replanning](/plans/errors-and-replanning).
