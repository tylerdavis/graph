...
Steps run **sequentially**; each input may reference plan inputs and any earlier step:
...
- The tighter the output spec, the more consistent the report run-over-run. For strict formats, write the structure as a grammar — see `sprint_analysis` in the [cookbook](/plans/cookbook).
