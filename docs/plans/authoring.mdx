...
Steps run **sequentially**; each input may reference plan inputs and any earlier step. Tool names are namespaced catalog names, plus three bare reserved ones: `plan_and_execute`, [`exit`](/plans/escape-hatch), and [`decide`](/plans/branching).
