...
In the [workbench](/using/plan-workbench), a decide step renders as a `⑂` junction — the chosen branch's steps run under its colored guide, the other stays untouched:

<WorkbenchShot
  shot="branching-decide"
  caption="A decide fork after a run down its then branch: the inline step list nests under the blue then head; the single-call else was never rendered."
/>
...
The judgment is a forced-schema call returning `{verdict, reason}`; `then` runs only on a yes, and the model's reasoning is carried in the step result — `{{E2.reason}}` — for downstream steps and the finish to use. Point the `judge` role at a cheap model; it falls back to `default`. Add `model:` alongside `infer` to override the model for this one verdict — a `[models.named]` entry, a role name, or `default` (it may itself be a template); ignored without `infer`.
