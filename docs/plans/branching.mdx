...
Exactly one of `when` or `infer` is required — unlike `exit`, where omitting both means "unconditionally". The gate grammar is identical to [exit gates](/plans/escape-hatch): `when` is a logical comparison (`eq ne gt lt gte lte empty not_empty contains`, with [typed splice](/plans/template-language#typed-splice) keeping numbers numeric), `infer` is a yes/no question judged by the `judge` model role. A `when` gate costs zero inference; an `infer` gate costs one cheap judgment call.
...
```yaml
input:
  when: { value: "{{E0.issues.length}}", op: gt, to: 0 }
  then:
...
| cost | `when`: 0 inference; `infer`: 1 judge call — plus whatever the branch calls |
