...
In the [workbench](/using/plan-workbench), a map renders as a `⟳` junction with one cyan body row — every iteration lands on that row, like a breakpoint on a loop line:

<WorkbenchShot
  shot="iteration-map"
  caption="A map paused mid-run by a loop-line breakpoint: the first iteration done, the second awaiting the debugger's decision on the one structural body row."
/>
