...
Add `model:` to override the model for this verdict — any configured [role](/models/models-and-providers#roles), standard or custom, or `default`. It defaults to the `judge` role. Use it to pin a specific gate to a cheaper or stronger model without changing the global `judge` role:
