...
[`[pricing]`](/reference/configuration#pricing--per-model-token-prices) is
configured — in the run log, just above the outcome line. Chat turns report
theirs the same way, so the cost of iterating on a draft is visible where the
iterating happens rather than only after the fact.
