...
Select a row in the plan tab and press `Enter` (or `e`, or double-click it) to open it in a form. A step's form has one field per thing the step carries: `id`, `tool`, `reasoning`, then one field per key of the tool's input schema — pre-filled from the draft, required keys first, the schema's description as the hint — plus any input keys the schema doesn't list, and, for tools whose schema is open, an `additional input` object for keys you want to add (control steps and tools declaring `additionalProperties: false` take no extra keys, so they don't get one). Control steps get their own vocabulary the same way: an `exit` opens with `status`, `when`, `infer`, `message`, and `model`. Steps inside `decide`/`map`/`reduce` bodies open too and write back into the owning step. The plan row at the top opens the metadata form: identifier, name, description, exemplars, required servers, input schema, and a `finish` select (solver, output, or silent) that shows only that mode's fields — the solver's query, system prompt, and data map, or the output template map.
...
| `Ctrl+T` | form | validate the edit without saving |
...
| `Ctrl+S` | anywhere | save the draft (in an editor: submit; in a form: save the edit) |
| `Esc` | editor / form | cancel (returns to the pause when injecting; discards the form's changes) |
