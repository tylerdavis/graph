---
title: "Workbench"
description: "The Graph development environment"
icon: "table-columns"
---
...
The workbench needs an interactive terminal; for scripting, use `graph plan run`.

<Frame caption="pr_review loaded and valid: the chat agent on the left, the plan's step tree and detail pane on the right.">
  ![pr_review loaded and valid: the chat agent on the left, the plan's step tree and detail pane on the right.](/images/workbench/workbench-overview.svg)
</Frame>
...
<Frame caption="The context tab: the namespaced catalog with read-only markers, and the selected tool's description and schemas in the detail pane.">
  ![The context tab: the namespaced catalog with read-only markers, and the selected tool's description and schemas in the detail pane.](/images/workbench/context-tab.svg)
</Frame>
...
<Frame caption="Drafting mid-flight: two steps accepted, the third being drafted, the outline's last stage still pending.">
  ![Drafting mid-flight: two steps accepted, the third being drafted, the outline's last stage still pending.](/images/workbench/drafting-overlay.svg)
</Frame>
...
## Editing steps and metadata

Select a row in the plan tab and press `Enter` (or `e`, or double-click it) to open it in a form. A step's form has one field per thing the step carries: `id`, `tool`, `reasoning`, then one field per key of the tool's input schema — pre-filled from the draft, required keys first, the schema's description as the hint — plus any input keys the step already carries that the schema doesn't list. Control steps get their own vocabulary the same way: an `exit` opens with `status`, `when`, `infer`, `message`, and `model`. Steps inside `decide`/`map`/`reduce` bodies open too and write back into the owning step. The plan row at the top opens the metadata form: identifier, name, description, exemplars, required servers, input schema, and a `finish` select (solver, output, or silent) that shows only that mode's fields — the solver's query, system prompt, and data map, or the output template map.

<Frame caption="The step form for pr_review's exit gate: schema-driven fields with their hints, the required status first, the when condition as JSON, and a scrollbar because the form is taller than the window.">
  <img src="/images/workbench/step-form.svg" alt="The step form for pr_review's exit gate: schema-driven fields with their hints, the required status first, the when condition as JSON, and a scrollbar because the form is taller than the window." />
</Frame>

`a` opens a blank step form to insert a step after the selected one, `A` before it; on the plan row it inserts at the start, and on a step inside a body it anchors on the owning top-level step. The form starts on the tool select with the next free `E` id filled in, and the draft is untouched until you submit.

Each field's title says how its text is read: `text` is taken verbatim; `json` is parsed, and a parse error pins to the field; `json or text` tries JSON first and falls back to the literal, so a template like `{{E0.items}}` and a number like `0` both land as intended; `one per line` builds a list. Empty fields are omitted from the step; a required one blocks saving.

Inside a form, `Tab` or `Enter` moves to the next field and `Shift+Tab` to the previous one; `Up`/`Down` cross a field boundary at its first or last line; `Shift+Enter` (or `Alt+Enter`, which works in every terminal — the workbench turns on the keyboard-enhancement protocol where the terminal supports it so `Shift+Enter` is distinguishable from `Enter`) inserts a newline in a multi-line field, which scrolls to keep the cursor visible; `PgUp`/`PgDn` or the wheel scroll the whole form when it is taller than the window, and a click focuses a field. `Ctrl+T` validates the edit without applying it, `Ctrl+S` submits it to the draft, `Esc` discards the changes — asking `y`/`n` first when there are unsubmitted ones.

Validation and submission follow the same rule as the chat editing tools: the edit is accepted only when it introduces no **new** static problems (a duplicate id, a forward reference…); problems the plan already had are shown dimmed and never block. `Ctrl+T` also reports catalog problems, such as a tool name that doesn't resolve, marked non-blocking because, as with the chat tools, they never stop a submit. A submitted edit replaces the draft — marked unsaved, undoable with `u`, written to disk by the next `Ctrl+S` outside the form — and re-validates it; renaming a top-level step id rewrites the downstream `{{id.*}}` references (a body step renamed while something still reads it is rejected). The `tool` field is a select over the catalog: type to filter, `Up`/`Down` to move through the matches, `Enter` or `Tab` to pick. Choosing a different tool reloads the input fields from the new tool's schema and keeps the values you already typed — keys the new schema doesn't list stay as their own fields, so nothing is lost silently.
...
Each run closes with what it spent — calls, tokens, and cost when [`[pricing]`](/reference/configuration#pricing--per-model-token-prices) is configured — in the run log, just above the outcome line. Chat turns report theirs the same way, so the cost of iterating on a draft is visible where the iterating happens rather than only after the fact.

<Frame caption="The run tab after a solver plan completes: the step transcript, the streamed answer, and the outcome line.">
  ![The run tab after a solver plan completes: the step transcript, the streamed answer, and the outcome line.](/images/workbench/run-tab.svg)
</Frame>

The agent can start any of these, and `run_plan` accepts `breakpoints` (top-level step ids, implies a debug run) — but **gated runs always pause for _your_ y/s/a decision on every tool call**: the agent starts the run and cannot approve its side effects.
...
<Frame caption="A gated pr_review run paused at a breakpoint on E5 — steps E0–E4 executed, the debug panel showing the rendered input about to be sent.">
  ![A gated pr_review run paused at a breakpoint on E5 — steps E0–E4 executed, the debug panel showing the rendered input about to be sent.](/images/workbench/pr-review-debug.svg)
</Frame>
...
| `Enter` | chat | send (`Shift+Enter` or `Alt+Enter` inserts a newline; pasted newlines never submit) |
| `j` / `k` | workspace | select a step or tool |
| `Enter` / `e` | plan tab | edit the selected step in a form (the plan row: its metadata) |
| `a` / `A` | plan tab | add a step after / before the selected one |
| `Tab` / `Shift+Tab` | form | next / previous field (`Up`/`Down` at a field's edge do the same) |
| `Ctrl+T` | form | validate the edit without applying it |
...
| `Ctrl+S` | anywhere | save the draft (in an editor or form: submit) |
| `Esc` | editor / form | cancel (returns to the pause when injecting; a form with unsubmitted changes asks first) |
...
| double-click a step row | open it in the edit form |
...
A left click anywhere closes the help overlay. While an editor modal is open the mouse is inert (the keyboard owns it); inside a form, a click focuses the field under the cursor and the wheel scrolls the form. Capturing the mouse means the terminal's own click-drag text selection is suspended — hold **Shift** (or **Option** on macOS) to select and copy text as usual.
