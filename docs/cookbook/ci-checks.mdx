...
- **E3** — one `infer` gate asking a two-part question: does the diff change a file's schema at all (a new optional field counts, since every schema change bumps the generation), _and_ is it unaccounted for? A removed or renamed key is caught mechanically by the crate's frozen fixture tests anyway, so the judge earns its keep on the cases fixtures cannot see — an added field, or a field whose meaning or accepted values changed with its shape intact.

Worst case per PR: one extra `judge` call, and only on PRs that touch a model file.
...
- **Cost per PR**: worst case two `judge`-role calls (docs-drift, and format-drift only when a model file changed) plus two review-sized calls — the review itself, and the transition audit only when prior findings exist (the audit carries full file contents alongside the diff, since it must judge a fix from the file's current state rather than from a hunk; the review itself uses a wide-context diff instead). A first review pays for the review alone — the `decide` skips the audit outright; drafts pay nothing.
- **One job per plan, no `needs` between them.** The jobs are independent and run in parallel; only the two drift gates can fail the check (exit 4). The reviewer always exits 0 — its comments are the deliverable, not a gate.
- **The pinned image reads the repo's own files.** A fourth job runs `graph config check` and `graph plan validate` over `./.graph/` inside the same pinned image, with no model calls. Plans and config load from the PR checkout while the engine comes from the image, so a [file version](/reference/file-versions) the image cannot read fails here until the pin moves in the same PR — the check that keeps the two from drifting apart.
