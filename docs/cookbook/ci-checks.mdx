  ...
  - id: E4
    ...
    input:
      ...
      do:
        ...
        input: { path: "{{item}}", ref: "{{E0.head_sha}}", max_bytes: 30000 }
  - id: E5
...
- **Ground absence claims in the complete file list, never the diff.** The diff is byte-capped and git orders hunks by path, so on a big PR the `docs/` hunks are exactly what falls off the end — an early version reported "docs not updated" false positives on a PR whose docs edits sat past the truncation point. The fix is layered: `git_changed_files` (E1) feeds the reviewer a complete, never-truncated file list, the `map` over `git_file` (E4) gives it whole files rather than hunks, `git_diff` appends a visible `[diff truncated: …]` marker when it cuts, and the rubric judges docs-parity and test-parity only from the complete list.
