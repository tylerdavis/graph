...
## `graph_review_9000` — a self-contained reviewer

The plan fetches its own context, reviews across eight passes, and maintains its findings as living review threads: prior findings are read back with their thread states, audited for transitions (fixed in code → resolved with a reply, dismissed by a human → declined), and fed to the reviewer as known context, so only genuinely new findings post as inline comments — open threads and human replies are never deleted. A summary comment with lifecycle counts refreshes in place. The CI step is `graph plan run graph_review_9000 --input pr="$PR"` — nothing else:
...
steps:
  ...
  - id: E3b
    tool_name: filter                 # deleted files have no contents at head
    input:
      over: "{{E1.changes}}"
      where: { value: "{{item.status}}", op: ne, to: deleted }
  - id: E4
    tool_name: map                    # full file contents at head, 8 at a time
    input:
      over: "{{E3b.items}}"
      concurrency: 8
      do:
        tool_name: builtin__git_file
        input: { path: "{{item.path}}", ref: "{{E0.head_sha}}", max_bytes: 30000 }
  - id: E4b
    tool_name: builtin__gh_pr_review_threads  # prior findings + thread states
    input: { pr: "{{input.pr}}", marker: "graph_review_9000:finding" }
  - id: E4c
    tool_name: builtin__infer         # transition audit: resolve / decline / none
    input:
      instruction: |
        ... decide, for each open finding, whether the file contents at head
        show it fixed (resolve — a mid-PR fix leaves no diff hunk, and an
        empty diff means everything was reverted), a human reply dismisses
        it (decline), or it stays open — insufficient evidence keeps it open ...
      output_schema: { type: object, properties: { findings: ... } }
  - id: E4d
    tool_name: builtin__gh_pr_thread_sync   # transitions only, before the gate
    input:
      pr: "{{input.pr}}"
      head_sha: "{{E0.head_sha}}"
      marker: "graph_review_9000:finding"
      creates: []
      threads: "{{E4c.findings}}"
  - id: E3
  ...
  - id: E4e
    tool_name: builtin__infer         # cluster the diff into functional groups
    input:
      instruction: |
        ... 1-6 coherent functional changes, each with a name, a focus,
        newline-joined file_list / symbol_list, and scoutworthy: false
        for prose/prompt/docs/config-only groups ...
      output_schema: { type: object, properties: { groups: ... } }
  - id: E4e2
    tool_name: filter                 # scouts only where code symbols changed
    input:
      over: "{{E4e.groups}}"
      where: { value: "{{item.scoutworthy}}", op: eq, to: true }
  - id: E4g
    tool_name: builtin__git_file      # CLAUDE.md at head: orientation for scouts
    input: { path: "CLAUDE.md", ref: "{{E0.head_sha}}", max_bytes: 30000 }
  - id: E4f
    tool_name: map                    # one context scout per group, 4 at a time
    input:
      over: "{{E4e2.items}}"
      concurrency: 4
      do:
        tool_name: agent              # callers, impls, tests for this group
        input:
          prompt: |
            ... find what {{item.name}} touches but does not show — callers,
            implementations, tests of {{item.symbol_list}} — at the head sha ...
          tools: ["builtin__git_grep", "builtin__git_file"]
          max_iterations: 18
          output_schema: { type: object, properties: { group: ..., entries: ..., notes: ... } }
  - id: E5
    tool_name: builtin__infer         # the rubric: eight passes + output schema
    input:
      instruction: |
        You are a principal engineer ... (the eight passes, inline, plus the
        known findings from E4c: report only genuinely NEW findings)
      output_schema: { type: object, properties: { passes: ..., findings: ... } }
  - id: E6
        ...
        input:
          shape:
            ...
            body: |
              ...
              > {{item.explanation}}
  - id: E7
    tool_name: builtin__gh_pr_thread_sync   # creates only; E4d did transitions
    input:
      pr: "{{input.pr}}"
      head_sha: "{{E0.head_sha}}"
      marker: "graph_review_9000:finding"
  ...
  - id: E8
    ...
    input:
      ...
      body: |
        > [!{{E5.overall_alert_type}}]
        > **graph_review_9000** · {{E5.overall_framing}}

        | Pass | Area | Status |
        |---|---|---|
        {{#E5.passes}}| {{number}} | {{area}} | {{status_emoji}} {{status_label}} |
        {{/E5.passes}}

        🧵 {{E7.created_count}} new · {{E7.still_open_count}} open ·
        {{E7.resolved_now_count}} resolved · {{E7.declined_now_count}} declined
```
...
- **The rubric can live in the plan.** `builtin__infer` accepts both the instruction and a caller-supplied `output_schema`, so the whole reviewer is one plan file with no repo-local tool to keep in sync. (The retired `pr_review` split them into a `./.graph/tools/` prompt tool — still the right shape when several plans share one rubric.)
...
- **Ground absence claims in the complete file list, never the diff.** The diff is byte-capped and git orders hunks by path, so on a big PR the `docs/` hunks are exactly what falls off the end — an early version reported "docs not updated" false positives on a PR whose docs edits sat past the truncation point. The fix is layered: `git_changed_files` (E1) feeds the reviewer a complete, never-truncated file list, the `filter` (E3b) drops deleted entries whose contents can't exist at head — so the `map` over `git_file` (E4) reads whole files rather than hunks without dying on a deletion, and its results stay index-aligned with `{{E3b.items}}` by construction — `git_diff` appends a visible `[diff truncated: …]` marker when it cuts, and the rubric judges docs-parity and test-parity only from the complete list.
...
- **Resolution is judged against the code, not against re-detection.** A dedicated audit call (E4c) transitions an open finding only when the code at head shows the fix, and treats insufficient evidence as "stays open" — if it instead resolved whatever the reviewer failed to re-find, one flaky review pass would silently close real findings. Declines come only from explicit human replies, never from silence. The reviewer (E5) then sees the known findings purely as a suppression list: report only what's new.
...
- **Thread hygiene runs before the nothing-to-review gate.** The pull → audit → transitions sync (E4b/E4c/E4d) sit ahead of the empty-diff exit, and the sync's two halves are split across it (E4d transitions, E7 creates). A fully reverted PR has an empty diff — exactly the case where every open finding just got fixed — so gating the lifecycle behind the exit would strand resolved-in-practice threads open forever, blocking merges on repos that require conversation resolution.
...
- **Cost per PR**: worst case one `judge`-role call (docs-drift) plus two review-sized calls — the transition audit and the review itself, each carrying full file contents alongside the diff (the audit needs the same grounding to verify a fix; on a first review it carries the context and an empty findings list); docs-only PRs pay only the review side; drafts pay nothing.
