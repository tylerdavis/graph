...
- **E2/E3** — the diff over the files that hold the version constants, and `git_changed_files` on `docs/reference/file-versions.mdx` alone, so the judge can see whether the change was accounted for.
- **E4** — one `infer` gate asking a two-part question: does the diff change a file's schema at all (a new optional field counts, since every schema change bumps the generation), _and_ is it unaccounted for? A removed or renamed key is caught mechanically by the crate's frozen fixture tests anyway, so the judge earns its keep on the cases fixtures cannot see — an added field, or a field whose meaning or accepted values changed with its shape intact.
