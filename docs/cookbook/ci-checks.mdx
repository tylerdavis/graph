...
- **E4** — one `infer` gate asking a two-part question: is the model change non-additive, _and_ is it unaccounted for? Additive changes (a new optional field with a default, a new enum variant) answer no by construction; a removed or renamed key is caught mechanically by the crate's frozen fixture tests anyway, so the judge's job is the case fixtures cannot see — a field whose meaning or accepted values changed with its shape intact.
