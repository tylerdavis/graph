...
This repo dogfoods graph in its own pull-request checks: a **docs-drift gate** (an inferred exit gate as a merge check), a **format-drift gate** (the same shape, guarding the [file versions](/reference/file-versions)), and a **specialized reviewer** (structured findings posted as PR comments by the plan itself). Every piece ships in this repo — the plans in `./.graph/plans/`, the repo-local tools in `./.graph/tools/`, the workflow in `.github/workflows/graph-checks.yaml` — and every PR against graph runs them.
...
## `format_drift` — the same gate, for the file versions

`.graph/plans/format_drift.yaml` guards the rule behind [file versions](/reference/file-versions): any schema change to a versioned file's model — the config structs, the plan and control-step grammars, tool documents, the store layout — must raise the matching `*_FORMAT` constant in the same PR (the changelog entry the release cuts is the record of what changed). Same shape as `docs_drift`, with the short-circuit and the judge's evidence swapped:

- **E0/E1** — `builtin__git_diff` scoped to the model files, and a `when: { op: empty }` exit. A PR that never touches a model file passes with zero LLM calls, which is nearly every PR.
- **E2** — the diff over the files that hold the version constants, so the judge can see whether the change was accounted for.
- **E3** — one `infer` gate that decides per file kind: is this kind's constant raised in the format diff? If so it is accounted for, whatever the model diff contains, and the judge moves on; only then does it ask whether the model diff changes the file's schema at all (a new optional field counts, since every schema change bumps the generation). The order matters: the first version asked "schema change _and_ unaccounted for?" as one conjunction, and the `judge` model answered yes to a PR whose own bump it had just described as accounting for the change — the size of the schema change outweighed the second clause. Checking the bump first removes that judgment call. A removed or renamed key is caught mechanically by the crate's frozen fixture tests anyway, so the judge earns its keep on the cases fixtures cannot see — an added field, or a field whose meaning or accepted values changed with its shape intact.

The gate pins its verdict to the `reviewer` role (`model: reviewer`) rather than the cheap `judge`: telling a schema change apart from a changed description string or doc comment in the same struct is a reading-comprehension task the small judge model got wrong on a real PR. Worst case per PR: one extra `reviewer`-role call, and only on PRs that touch a model file.
...
- **`graph_review_core`** — the review engine, purely local (git only). Fetches the changed files and a diff carrying 25 lines of context around every hunk (whole file contents are deliberately not fetched — see the cost note below); clusters the diff into functional groups; scouts each scoutworthy group's blast radius with bounded [`agent` steps](/plans/agent-step); runs the eight-pass rubric; returns structured JSON (`output:` finish — framing, a severity-ordered pass table, findings). It takes previously known findings as a suppression list, so callers with history get only what's new.
...
- **Cost per PR**: worst case one `judge`-role call (docs-drift), one `reviewer`-role call (format-drift, only when a model file changed), plus two review-sized calls — the review itself, and the transition audit only when prior findings exist (the audit carries full file contents alongside the diff, since it must judge a fix from the file's current state rather than from a hunk; the review itself uses a wide-context diff instead). A first review pays for the review alone — the `decide` skips the audit outright; drafts pay nothing.
- **One job per plan, no `needs` between them.** The three jobs are independent and run in parallel; only the two drift gates can fail the check (exit 4). The reviewer always exits 0 — its comments are the deliverable, not a gate.
