...
This repo dogfoods graph in its own pull-request checks: a **docs-drift gate** (an inferred exit gate as a merge check), a **format-drift gate** (the same shape, guarding the [file formats](/reference/formats)), and a **specialized reviewer** (structured findings posted as PR comments by the plan itself). Every piece ships in this repo — the plans in `./.graph/plans/`, the repo-local tools in `./.graph/tools/`, the workflow in `.github/workflows/graph-checks.yaml` — and every PR against graph runs them.
...
## `format_drift` — the same gate, for the file formats

`.graph/plans/format_drift.yaml` guards the rule behind [file formats](/reference/formats): any schema change to a versioned format's model — the config structs, the plan and control-step grammars, tool documents, the store layout — must raise the matching `*_FORMAT` constant and document the migration on the formats page in the same PR. Same shape as `docs_drift`, with the short-circuit and the judge's evidence swapped:
...
- **E2/E3** — the diff over the files that hold the format constants, and `git_changed_files` on `docs/reference/formats.mdx` alone, so the judge can see whether the change was accounted for.
