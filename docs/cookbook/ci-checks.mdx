...
steps:
  ...
  - id: E4c
    ...
    input:
      instruction: |
        ... decide, for each open finding, whether the code at head shows it
        fixed (resolve), a human reply dismisses it (decline), or it stays
        open — insufficient evidence keeps it open ...
        ...
        input:
          shape:
            ...
            body: |
              ...
              > {{item.explanation}}
  - id: E7
    tool_name: builtin__gh_pr_thread_sync   # post new, transition existing
    input:
      pr: "{{input.pr}}"
...
- **Resolution is judged against the code, not against re-detection.** A dedicated audit call (E4c) transitions an open finding only when the diff and file contents at head show the fix, and treats insufficient evidence as "stays open" — if it instead resolved whatever the reviewer failed to re-find, one flaky review pass would silently close real findings. Declines come only from explicit human replies, never from silence. The reviewer (E5) then sees the known findings purely as a suppression list: report only what's new.
