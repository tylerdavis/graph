...
The plan fetches its own context, reviews across eight passes, anchors findings as inline diff comments, and posts its own summary comment linking to them. The CI step is `graph plan run graph_review_9000 --input pr="$PR"` — nothing else:
...
steps:
  ...
  - id: E5
    ...
    input:
      instruction: |
        You are a principal engineer ... (the eight passes, inline)
      output_schema: { type: object, properties: { passes: ..., findings: ... } }
  - id: E6
    tool_name: map                    # findings -> alert-style comment bodies
    input:
      over: "{{E5.findings}}"
      concurrency: 8
      do:
  ...
  - id: E7
    ...
    input:
      ...
      marker: "graph_review_9000:inline"
      comments: "{{E6.results}}"
  - id: E8
...
- **Ground absence claims in the complete file list, never the diff.** The diff is byte-capped and git orders hunks by path, so on a big PR the `docs/` hunks are exactly what falls off the end — an early version reported "docs not updated" false positives on a PR whose docs edits sat past the truncation point. The fix is layered: `git_changed_files` (E1) feeds the reviewer a complete, never-truncated file list, the `filter` (E4) drops deleted entries whose contents can't exist at head — so the `map` over `git_file` (E4b) reads whole files rather than hunks without dying on a deletion, and its results stay index-aligned with `{{E4.items}}` by construction — `git_diff` appends a visible `[diff truncated: …]` marker when it cuts, and the rubric judges docs-parity and test-parity only from the complete list.
...
- **Findings flow through the pipeline, enriched at each step.** `reshape` (E6) adds `path`/`body` to each finding without dropping the original fields; `gh_pr_inline_comments` (E7) passes those extras through and adds `url` where a comment posted. So the blockers list (E8) renders from one array — severity and label from the review, the link from the post — with no join logic, which the [template dialect](/plans/template-language) couldn't express anyway. Findings the model couldn't pin to a line (or whose line the API rejects) simply come back without `url`, and the inverted section renders them as plain text.
- **Repeated runs refresh everything in place.** Both posting tools take a distinct `marker`, embed it as a hidden tag, and find-and-edit (or delete-then-repost) the comment carrying it. Each push updates the review instead of stacking. Prefer a marker even for a lone plan: without one `gh_pr_comment` falls back to `--edit-last`, which keys on the token identity, so the day a second plan comments on the same PR as the same bot the two summaries start overwriting each other. One marker per plan per comment surface.
...
- **Cost per PR**: worst case one `judge`-role call (docs-drift) plus one review call, the latter carrying full file contents alongside the diff; docs-only PRs pay only the review; drafts pay nothing.
