...
steps:
  ...
  - id: E4e
    tool_name: agent                  # context scout: callers, impls, tests
    input:
      prompt: |
        ... find what the diff touches but does not show — callers,
        implementations, tests of the changed symbols — at the head sha ...
      tools: ["builtin__git_grep", "builtin__git_file"]
      max_iterations: 12
      output_schema: { type: object, properties: { entries: ..., notes: ... } }
  - id: E5
...
- **Blast radius is scouted, not guessed.** The diff and changed-file contents can't show who *calls* what changed — so an [`agent` step](/plans/agent-step) (E4e) runs a bounded loop over read-only `git_grep`/`git_file`, chasing callers, implementations, and tests of the changed symbols at head, and hands the review a structured dossier. It sits after the empty-diff gate (it only earns its cost when there's something to review), and E5's template guards on `{{E4e.final}}`, so a scout that exhausts its budget degrades the review's evidence rather than failing the run.
