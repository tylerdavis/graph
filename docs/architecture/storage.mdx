...
```text
<data_dir>/FORMAT                        the store format version, written on first open
...
`FORMAT` is the store's [format version](/reference/formats#data-directory). graph writes it the first time it opens a data directory and refuses to open one whose marker is newer than the version it reads — an older binary must never append to a layout it does not understand.
