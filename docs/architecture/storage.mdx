...
```text
<data_dir>/FORMAT                        the store's file version, written on first open
...
`FORMAT` is the store's [file version](/reference/file-versions#data-directory). graph writes it the first time it opens a data directory and refuses to open one whose marker is outside the versions it reads — an older binary must never append to a layout it does not understand.
