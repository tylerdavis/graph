...
- Two control steps are intercepted by the executor, never dispatched to a tool: [`exit`](/plans/escape-hatch) ends the plan early, [`decide`](/plans/branching) forks into one of two branches. A `decide` step renders only its gate up front; the chosen branch renders after evaluation, and the non-taken branch never renders. Its gate evaluation and each branch call appear as ordinary flat tool events in traces.
...
| `plan run` (solver) | 1 |
| `exit` / `decide` gate | 0 (`when`) or 1 judge call (`infer`) |
