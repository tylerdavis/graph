...
Bounded by `max_agent_iterations`. Tool failures return _into_ the loop as error results — the agent explains or works around them visibly.
...
Call counts are what you can reason about up front. For what a run actually spent — tokens and dollars, broken down by step and by model — see [what a run spent](/reference/scripting-contract#what-a-run-spent).

## Model roles

Every inference site resolves through a `[models.<role>]` entry (the standard roles `chat`, `planner`, `solver`, `repair`, `judge` fall back to `default`), so cost tuning is pure config: strong model where judgment lives, fast model where volume lives. [Custom roles](/models/models-and-providers#custom-roles) use the same shape and are selectable at the point of use — per-step model routing without touching the standard assignments. Structured outputs (planner, prompt tools) get one automatic `repair`-role fix-up attempt before erroring into the replan loop. The full map of roles, providers, and failover is [Models & providers](/models/models-and-providers).
