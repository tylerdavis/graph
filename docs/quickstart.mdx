...
<Tabs>
  <Tab title="Automated">
    ...
    Grab a binary, scaffold a project, and hand the setup to your coding agent. A [skill](#let-your-agent-finish-the-setup) walks the agent through wiring a provider, adding tools, and drafting a first plan against your own data — then you review it in the [workbench](/building/workbench) and run it. Prefer to do it by hand?

    <Steps>
      <Step title="Install graph">
        Grab the latest release binary — macOS (arm64) or Linux (x86_64). Each release publishes version-pinned tarballs with SHA-256 checksum:
      ...
      <Step title="Let your agent finish the setup">
        ...
        <Tip>
          The same `npx skills add` command installs two more: `/graph-plan-authoring`, which teaches the agent to build and repair plans with the [authoring commands](/plans/authoring#authoring-from-the-command-line), and `/graph-github-actions-setup`, which scaffolds a merge gate or PR reviewer as a plan — see [CI checks](/cookbook/ci-checks) for the worked example.
        </Tip>
      </Step>
      <Step title="Open it in the workbench">
        ...
        ```bash
        graph workbench plan project_status
        ```
        ...
        <Frame caption="A loaded plan in the workbench: chat agent left, step tree and detail right.">
          ![A loaded plan in the workbench: chat agent left, step tree and detail right.](/images/workbench/workbench-overview.svg)
        </Frame>
      </Step>
      <Step title="Run your first plan">
        Once it's right, run it:
  ...
  <Tab title="Manual">
    ...
    <Steps>
      ...
      <Step title="Probe the tools you'll build on">
        Before writing a step, see each tool's input schema and _actual_ output shape:
...
<Columns cols={2}>
  <Column>
    <Card title="Core concepts" icon="sparkles" href="getting-started/concepts">
      learn about the graph primitives
    </Card>
  </Column>
  <Column>
    <Card title="Add your MCP tools" icon="sparkles" href="tools/mcp-servers">
      Integrate with everything via MCP
    </Card>
  </Column>
</Columns>
