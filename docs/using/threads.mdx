...
Every successful `ask` and `chat` turn persists to a thread — the full message history including tool calls and their results. Threads live in the embedded database and survive across processes.
...
<Note>
Threads require the default `ladybug` storage backend. With `GRAPH_STORAGE=memory` everything works but nothing survives the process — the right trade for CI. See [Storage](/architecture/storage).
</Note>
