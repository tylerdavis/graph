...
`{{E0.teams.0.name}}` is the [template language](/plans/template-language) — typed dataflow between steps, zero inference. Inference appears only where the plan puts it: a [prompt tool](/tools/user-defined#prompt-an-llm-call-as-a-tool) step for a sub-task, and the [finish](/plans/finish-modes) — a solver report, structured JSON, or nothing.
...
This is what makes plans composable: authoring a plan *adds a tool to the catalog*, and a plan's steps draw from the same catalog — MCP tools, wrapped CLIs, saved queries, prompt tools.
