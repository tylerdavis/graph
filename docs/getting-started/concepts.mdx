...
Control flow lives in two executor-intercepted steps: an [`exit` gate](/plans/escape-hatch) ends the plan early with a success or error state, and a [`decide` step](/plans/branching) forks into one of two branches on the same gates. Both are gated logically (`when` on exit, `if` on decide) or by a cheap model judgment (`infer`).
