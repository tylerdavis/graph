...
`graph ask` and `graph chat` run an agent loop over the same tool catalog. Use it as a workbench: probe a tool's real output, prototype a workflow conversationally, then freeze what works into a plan. Details in [Chat & ask](/using/chat-and-ask).
...
<CardGroup cols={2}>
  ...
  <Card title="Core concepts" icon="lightbulb" href="/getting-started/concepts">
    Plans, the tool catalog, the shape cache, and what a run costs.
  </Card>
