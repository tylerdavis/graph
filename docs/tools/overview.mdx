...
- **Plan steps can call MCP and user tools** — and user tools can wrap anything executable, so plans can orchestrate arbitrary local commands alongside API calls.
- **Plans cannot call plans.** Plan steps execute against the base catalog (MCP + user + built-ins), which keeps execution non-recursive and predictable.
