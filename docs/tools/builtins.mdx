...
Ticket identifier (e.g. `LN-1234`) from a PR's metadata — deterministic regex extraction, zero LLM calls, so plans don't burn an inference on what a pattern match can do. Scans the head branch name, then the title, then the body; first match wins and is normalized to uppercase-with-dash form (`ln1234`, `LN 1234`, `ln-1234` → `LN-1234`). Read-only.

| Input | Type |  |
| --- | --- | --- |
| `pr` | integer | required |
| `pattern` | string | optional — POSIX extended regex the ID must match; default `[A-Za-z]{2,6}[- ]?[0-9]+` covers common tracker forms |
