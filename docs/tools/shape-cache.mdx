...
The cache is read fresh at each planning attempt — a shape observed earlier in the _same run_ (an agent tool call, a prior step) is already available to the next plan.
...
- **Cold caches degrade, not break.** With no shape, the planner is instructed to use whole-result references (`{{E0}}`) or stop and continue planning after real results exist; a wrong deep path fails with a digest of the keys that _do_ exist, and one replan fixes it.
